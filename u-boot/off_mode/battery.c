...
// use 661_2_081011.INI.txt for LiCo battery pack
char lico_new_data0[] = {0xA8,0x90,0xAE,0xC0,0xB5,0x00,0xB7,0xC0,
		 0xBB,0xA0,0xBB,0xF0,0xBC,0x50,0xBC,0xA0};

char lico_new_data1[] = {0xBC,0xF0,0xBE,0x60,0xBF,0xD0,0xC1,0x30,
		 0xC5,0x50,0xC8,0x50,0xCC,0x60,0xD0,0x70};

char lico_new_data2[] = {0x04,0xF0,0x04,0xE0,0x0B,0x00,0x06,0x00,
		 0x45,0xF0,0x3A,0x40,0x3F,0x10,0x3F,0x10};

char lico_new_data3[] = {0x0F,0xD0,0x0F,0xD0,0x10,0xE0,0x07,0xF0,
		 0x0C,0x20,0x07,0xE0,0x07,0xE0,0x07,0xE0};
...
static int ds2786_battery_rsoc()
{
	...
	rsoc_lsb = rsoc_lsb * 4 /10;
	//rsoc_lsb = (((rsoc_msb & 0x01)<< 8) | rsoc_lsb)/256;
...
int max17043_write_custom_model( void )
{
	...
	if(g_BatteryID != BATTERY_ID_MCNAIR)
	{
		// SOCCheckA = 118, SOCCheckB = 120
		//if(soc[0]>=0xE2 && soc[0]<=0xE4)
		if(soc[0]>=0x76 && soc[0]<=0x78)
