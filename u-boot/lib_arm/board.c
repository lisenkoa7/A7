...
//&*&*&*SJ1_20110408, Add bootup logo.
#if defined (CONFIG_BOOTUP_LOGO)
	{
		...
		mmc_init(1);
		sprintf(cmd, "mmc 1 read %x 1400000 1d0000", (LCD_FB_PHY_ADDR-0x40));	
		run_command (cmd, 0);
...
//20111101, JimmySu remove bootup environment judgement
//			if ((PB_env == 1) || (INITRAMFS_env == 1)) {
			if (PB_env == 1) {
//&*&*&*SJ1_20111123, check 'debug' environment to turn on/off debug message.
#if defined (CONFIG_DEBUG_SWITCH)
				if (0 == Debug) {
					result_env = getenv("bootargs");
					p = strstr(result_env, ",115200n8");
