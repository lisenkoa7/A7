...
int gpio_init (void)
{
	...
	/** Enable LCD_PWR_EN GPIO_109 **/
	sr32((u32)&gpio4_base->oe, 15, 1, 0);
	sr32((u32)&gpio4_base->setdataout, 15, 1, 1);
	
	/** Set KEY_LIGHT_EN GPIO_140 as output low **/
    sr32((u32)&gpio5_base->oe, 12, 1, 0);
	sr32((u32)&gpio5_base->setdataout, 12, 1, 0);
...
//&*&*&*SJ1_20110607, Add check 3G modem exist GPIO status for EVT. 
int check_3G_exist(void)
{
	gpio_t *gpio5_base = (gpio_t *)OMAP34XX_GPIO5_BASE;
	u32 value, reg;
	
	/** Get 3G-EXIST GPIO_143 **/
	value = read_gpio_value((u32)&gpio5_base->datain, 15);
	reg = __raw_readl(0x48002170);	/* McBSP3_FSX, GPIO_143 */
	reg &= ~(1<<20); /* Set PullDown selected. */
	__raw_writel(reg, 0x48002170);

	return (value);
}
//&*&*&*SJ2_20110607, Add check 3G modem exist GPIO status for EVT. 
...
#define MUX_DEFAULT_ES2()\
	...
	/*GPMC*/\
	MUX_VAL(CP(GPMC_A1),        (IDIS  | PTD | EN | OFF_IN_PD | M4)) /*GPIO-34, CHG_OFFMODE*/\	
	MUX_VAL(CP(GPMC_A2),        (IDIS | PTD | DIS | M4)) /*GPIO-35, 3G SYSTEM_RESET_N */\
	MUX_VAL(CP(GPMC_A3),        (IDIS | PTD | EN |  M4)) /*GPIO-36 MODEM_POW_EN, enable pulled down to avoid inrush current*/\
	MUX_VAL(CP(GPMC_A4),        (IDIS | PTD | EN   | OFF_OUT_PD | M4)) /*GPIO-37, LVDS SHTDN_N*/\
	MUX_VAL(CP(GPMC_A5),        (IDIS | PTD | EN   | OFF_OUT_PD | M4)) /*GPIO-38, LCD_BL_EN */\
	MUX_VAL(CP(GPMC_A6),        (IEN  | PTD | DIS  | OFF_IN_PD | M4)) /*GPIO-39*/\
	MUX_VAL(CP(GPMC_A7),        (IDIS | PTU | DIS | OFF_IN_PU | M4)) /*GPIO-40, WL_XTAL_PU */\
	...
	MUX_VAL(CP(GPMC_A10),       (IEN  | PTD | DIS | OFF_IN_PD | M4)) /*GPIO-43, CHG_FAULT */\
	...
	MUX_VAL(CP(GPMC_D8),        (IEN  | PTU |  EN  | OFF_IN_PU | M4)) /*GPIO-44, VOLT_INT, remain same state in sleep*/\
	MUX_VAL(CP(GPMC_D9),        (IDIS | PTU | EN | OFF_IN_PU | M4)) /*GPIO-45, CCM_PWDN*/\
	MUX_VAL(CP(GPMC_D10),       (IDIS | PTD | DIS | M4)) /*GPIO-46, CCM_RESET_N*/\
/*	MUX_VAL(CP(GPMC_D11),       (IDIS | PTD | EN | OFF_IN_PD | M4)) *//*GPIO-47, SYS_nIRQ */\
	...
	MUX_VAL(CP(GPMC_D14),       (IEN | PTU | EN | OFF_IN_PU| M4)) /*GPIO-50, CCM_2V8_EN*/\
	...
	MUX_VAL(CP(GPMC_nWP),       (IDIS | PTD | EN  | OFF_IN_PD | M4)) /*GPI0-62*/\
	...
	MUX_VAL(CP(DSS_DATA20),     (IDIS | PTD | DIS | OFF_IN_PD | M0)) /*DSS_DATA20*/\
	...
	MUX_VAL(CP(CAM_D0 ),        (IEN  | PTU | DIS | M4)) /*GPIO-99, HPH_DETECT*/\
	MUX_VAL(CP(CAM_D1 ),        (IEN  | PTD | EN  | OFF_IN_PD | M4)) /*GPIO-100*/\
	MUX_VAL(CP(CAM_D2 ),        (IDIS | PTD | EN  | OFF_IN_PD | M4)) /*GPIO-101, VGH_CTRL*/\
	MUX_VAL(CP(CAM_D3 ),        (IEN  | PTD | EN  | OFF_IN_PU | M4)) /*GPIO-102, WL_OOB_INTR*/\
	...
	MUX_VAL(CP(CAM_D10),        (IDIS | PTD | EN  | OFF_IN_PD | M4)) /*GPIO-109, LCD_PWR_EN*/\
	MUX_VAL(CP(CAM_D11),        (IDIS | PTD | EN  | OFF_IN_PD | M4)) /*GPIO-110, LVDS_CLKSEL*/\
	...
	/*Bluetooth*/\
	MUX_VAL(CP(McBSP3_DX),      (IDIS | PTD | DIS | OFF_IN_PD | M4)) /*McBSP3_DX, KEY_LIGHT_EN  */\
	MUX_VAL(CP(McBSP3_DR),      (IDIS  | PTU | EN  | OFF_OUT_PU | M4)) /*McBSP3_DR, LCD_RST */\
	MUX_VAL(CP(McBSP3_CLKX),    (IDIS | PTU | EN | OFF_OUT_PU | M4)) /*McBSP3_CLKX, LCD_STB*/\
	MUX_VAL(CP(McBSP3_FSX),     (IEN  | PTD | DIS | OFF_IN_PD | M4)) /*McBSP3_FSX, 3G_det */\
	/*UART2 & 3  Interface */\
	...
	MUX_VAL(CP(McBSP1_CLKR),    (IDIS | PTD | EN  | OFF_IN_PD | M4)) /*GPIO-156, OTG_EN_1V8*/\
	MUX_VAL(CP(McBSP1_FSR),     (IDIS | PTD | DIS | OFF_IN_PD | M4)) /*GPIO_157, TOUCH_RST*/\
	MUX_VAL(CP(McBSP1_DX),      (IDIS | PTD | DIS | OFF_IN_PD | M4)) /*GIO-158, WL_RST_N*/\
	MUX_VAL(CP(McBSP1_DR),      (IDIS | PTD | DIS | OFF_IN_PD | M4)) /*GIO-159, TOUCH_EN*/\
	...
	MUX_VAL(CP(JTAG_EMU0),      (IEN  | PTD | DIS | OFF_IN_PU/*OFF_IN_PD*/ | M4)) /*GIPO-11, UP_IOC_DATA_GPIO, Herry modify*/\
	...
	MUX_VAL(CP(ETK_D0_ES2 ),    (IEN  | PTU | EN  | OFF_IN_PD | M4)) /*GIO-14, TOUCH_INT */\
	...
	MUX_VAL(CP(ETK_D7_ES2 ),    (IDIS | PTD | DIS | M4)) /*GIO-21, W_DISABLE_N) */\
	MUX_VAL(CP(ETK_D8_ES2 ),    (IDIS | PTU | EN | OFF_OUT_PU/*| OFF_OUT_PD*/ | M4)) /*WL_POW_EN, GIO-22 Always ON*/\
...
void check_second_bootloader(void)
{
	...
	run_command("fatload mmc 0:1 80f80000 cdpg-second-bootloader.ep5a", 0);
