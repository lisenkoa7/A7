		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val == 0)
	{
		lsb = lsb | (MAX_LOW_BATTERY_THRESHOLD - (chip->low_battery+1));
		value = (msb<<8) | lsb;
	}
	else
	{
		lsb = lsb | (MAX_LOW_BATTERY_THRESHOLD - (val+1));
		value = lsb;
	}
...
static void max17043_set_low_battery(struct i2c_client *client, u8 value)
{
	...
	*/
	...
	config_value = (config_value << 8) | (MAX_LOW_BATTERY_THRESHOLD-(value+1));
...
static void max17043_set_low_battery_alert(struct max17043_chip *chip)
{
	if(chip)
	{
		//&*&*&*HC1_20111122, Mod low battery wakeup condition (only 3% alert)	
		#if 0
		...
		chip->low_battery = SHUT_DOWN_BATTERY_THRESHOLD;			
		#endif
		//&*&*&*HC2_20111122, Mod low battery wakeup condition (only 3% alert)	
	}
}
