...
#define EDP_LCD_RESET 		141
#define EDP_LCD_STBYB					142
#define EDP_LCD_VGH_CTRL				101
...
static int edp1_panel_enable_lcd(struct omap_dss_device *dssdev)
{
	...
	mdelay(50);		
		gpio_direction_output(EDP_LCD_RESET, 0);
...
static void edp1_panel_disable_lcd(struct omap_dss_device *dssdev)
{
	...
	mdelay(20);
	gpio_direction_output(EDP_LCD_PWR_EN, 0);//for vdd
	mdelay(30);
	gpio_direction_output(EDP_LCD_RESET, 1);
...
static struct omap_dss_device evt_lcd_device = {
	...
	.panel = {
                .width_in_mm = 154/*204*/, /*20111214, JimmySu modify for pass CTS */
                .height_in_mm = 90/*152*/, /*devices display should support square pixels*/
        },	
};
#else
static struct omap_dss_device evt_lcd_device = {
...
void __init evt_lcd_panel_init(void)
{
	...
	gpio_request(EDP_LCD_RESET, "lcd_panel_reset");
...
//power on  sequence	
	...
	mdelay(100);
	r2=gpio_direction_output(EDP_LCD_PANEL_BACKLIGHT_GPIO, 1);
*/
